...
class LaserLaneProposalEgo(Dataset):
    def __init__(self, data_root,  data_split_file, mode="valid", cfg=None):
        ...
        sub_lidar_path = 'lidar_ego'
        sub_label_path = 'labels_inside_lidar_range'
        image_path = osp.join(data_root, sub_img_path)
        lidar_path = osp.join(data_root, sub_lidar_path)
        ...
        orientation_path = osp.join(data_root, sub_label_path, 'spahaodi rse_orient')
        ...
        self.mode = mode
        self.cfg = cfg
        if cfg.use_lidar == True:
            self.lidar_list = [osp.join(lidar_path, lidar_stem + 'las') for lidar_stem in image_stem_list]
            self.lidar_pipeline = Pack3DDetInputs(keys=('points',))
    ...
    def __getitem__(self, idx):
        ...
        sample_propsal['prop_offset_mask'] = gt_offset_mask
         ...
         # ego lidar
        if self.cfg.use_lidar == True:
            lidar_pts = read_las(self.lidar_list[idx])
            # sample_propsal['lidar_pts'] = lidar_pts
            lidar_pts = BasePoints(lidar_pts, points_dim=4)  #
            # sample_propsal.update(points=dict(data=lidar_pts.tensor, cpu_only=False))           
            # sample_propsal.update(points=dict(points=lidar_pts, cpu_only=False))
            
            # lidar_pts_base = BaseDataElement()
            # lidar_pts_base.set_data(dict(points=lidar_pts))

            # lidar_pts_base = dict(points=lidar_pts, cpu_only=False)
            sample_3d = dict(points=lidar_pts)
            sample_3d = self.lidar_pipeline(sample_3d)
            # print('sample_3d: ', sample_3d)
            # sample_propsal.update(points=sample_3d['inputs']['points'])
            sample_propsal.update(points=BaseDataElement(data=sample_3d['inputs']['points'], cpu_only=False))
...
def load_image(image_path, mask_path, instance_path, ori_path=None, endp_path=None, downsample_instance=True, downsample_ratio=8):
    ...
    # for 2 classes, solid is 1, dashed is 2
