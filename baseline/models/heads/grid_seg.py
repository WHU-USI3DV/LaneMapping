...
class GridSeg(nn.Module):
        ...
        for k in range(num_of_labels):
            ...
            for i in range(144):
                for j in range(144):
                    ...
                    if self.cfg.dataset_type == 'LaserLane':
                        if line_num == 0:
                            # confidence
                            label_temp[y_idx][x_idx][1] = 0
                            # classification
                            label_temp[y_idx][x_idx][0] = self.cfg.heads.num_classes - 1
                        else: # 클래스
                            # confidence
                            label_temp[y_idx][x_idx][1] = 1
                            # classification
                            label_temp[y_idx][x_idx][0] = line_num - 1
                    else:
                        if line_num == 255:
                            label_temp[y_idx][x_idx][1] = 0
                            # classification
    ...
    def loss(self, out, batch):
        num_classes = self.cfg.heads.num_classes  # modified by Xiaoxin
        ...
        y_pred_cls = out[:, 0:num_classes, :, :]  # modified by Xiaoxin
        y_pred_conf = out[:, num_classes, :, :]   # modified by Xiaoxin
    ...
    def get_lane_map_numpy_with_label(self, output, data, is_flip=True, is_img=False):
        ...
        *       'rgb_img_conf_cls_idx': (144, 144, 3)
        ...
        num_classes = self.cfg.heads.num_classes  # number of lanes(0, 1, 2, ...) and the background (255)
        lane_maps = dict()
        ...
        for batch_idx in range(batch_size):
            ...
            if data_mode == 'LaserLane':
                cls_label = np.where(cls_label == 0, 255, cls_label)  # LaserLane
                cls_label = np.where(cls_label < 255, cls_label-1, cls_label)  # LaserLane
            ...
            cls_idx[np.where(cls_idx == (num_classes - 1))] = 255
